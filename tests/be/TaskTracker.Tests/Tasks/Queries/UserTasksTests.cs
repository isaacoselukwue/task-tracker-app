namespace TaskTracker.Tests.Tasks.Queries;[TestFixture]
internal class UserTasksTests
{
    private Mock<ITaskTrackerService> _mockTaskTrackerService;
    private UserTasksQuery invalidZeroPageNumberQuery;
    private UserTasksQuery invalidZeroPageCountQuery;
    private UserTasksQuery validQuery;
    private UserTasksValidator _validator;
    private UserTasksQueryHandler _handler;
    ...
    [Test]
    public async Task UserTasks_ZeroPageNumber_FailsValidation()
    {
    ...
    [Test]
    public async Task UserTasks_ZeroPageCount_FailsValidation()
    {
    ...
    [Test]
    public async Task UserTasks_ValidParams_PassesValidation()
    {
    ...
    [Test]
    public async Task UserTasks_EmptyResults_ReturnsEmptyList()
    {
        var emptyList = new List<UpcomingTasksResult>().AsAsyncQueryable();

        _mockTaskTrackerService.Setup(x => x.GetTasks()).Returns(emptyList);
    ...
    [Test]
    public async Task UserTasks_SinglePage_ReturnsCorrectPagination()
    {
        var testData = GetTestTasks(5).AsAsyncQueryable();

        _mockTaskTrackerService.Setup(x => x.GetTasks()).Returns(testData);
    ...
    [Test]
    public async Task UserTasks_MultiplePages_ReturnsCorrectPage()
    {
    ...
    [Test]
    public async Task UserTasks_LastPage_ReturnsRemainingItems()
    {
        var testData = GetTestTasks(25).AsAsyncQueryable();
